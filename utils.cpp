...
int utils::generateRandomInteger(const float min, const float max)
{
	...
	float random = static_cast<float>(distribution(gen));
	return int(((random / RAND_MAX) * range) + min);
}
float utils::generateRandomFloat(const float min, const float max)
{
	std::random_device rand;
	std::mt19937 gen(rand());
	std::uniform_int_distribution<> distribution(0, RAND_MAX);
	...
	float random = static_cast<float>(distribution(gen));
	return (((random / RAND_MAX) * range) + min);
}
float utils::GetDistanceBetweenPoints(const Point2f& p1, const Point2f& p2)
{
